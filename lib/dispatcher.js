...
var dispatcher = function (args) {
  ...
  if (!task) {
    taskObj = loadTask(function () {
      args.print.data('No task specified.');
      args.print.data('See "nt help" for usage or "nt list" for a list of available tasks.');
      return false;
    });
  }
  ...
  if (!ntrcFound && task === 'list') {
    return args.print.err('Couldn\'t find "ntrc" directory - nothing to list. See "nt help".');
  }
