...
module.exports = {
  ...
  getScreenDefinition: function (screen) {
    ...
    if (!config.resolved || !config.resolved.ntrc) {
      console.log('[nt] Config not found, can\'t generate screen');
      return {};
    }
    ...
    if (Object.keys(screenDefinitionLegacy).length) {
      // console.log('[nt] Using legacy tasks.(screen).json definitions. Please update to layout.json model as the legacy support is being phased out.');
